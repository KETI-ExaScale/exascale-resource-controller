...
import (
	...
	cloudcontrollers "github.com/KETI-ExaScale/exascale-resource-controller/controllers/cloud"
...
func main() {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "65b60567.exascale.keti",
	...
	if err = (&cloudcontrollers.ClusterReconciler{
	...
	if err = (&cloudcontrollers.NodeReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
	...
	if err = (&metriccontrollers.ScoreReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "Score")
		os.Exit(1)
	}
	if err = (&metriccontrollers.GPUReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
	...
	if err = (&metriccontrollers.CollectorReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
